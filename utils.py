...
def get_hyperparams(timesteps, schedule="cosine"):
    if schedule == "cosine":
    ...
    return {
        "alphas": alphas, "alphas_cumprod": alphas_cumprod, \
...
def p_sample_loop(hyperparams, model, shape):
    ...
    for i in tqdm(reversed(range(0, model.time_embedding.num_embeddings)), desc='Sampling loop time step', total=model.time_embedding.num_embeddings):
        img = p_sample(hyperparams, model, img, torch.full((b,), i, device=device, dtype=torch.long), i)
        imgs.append(img.cpu().numpy())
    return imgs

@torch.no_grad()
def sample(hyperparams, model, image_size, batch_size=16, channels=3):
    return p_sample_loop(hyperparams, model, shape=(batch_size, channels, image_size, image_size))

def sample_and_save(hyperparams, model, iteration, image_size, num=16):
        all_images_list = sample(hyperparams, model, image_size=image_size, batch_size=num,)
        all_images = torch.cat(all_images_list, dim=0)
        all_images = (all_images + 1) * 0.5
        save_image(all_images, f'sample-{iteration:06}.png', nrow = 4)